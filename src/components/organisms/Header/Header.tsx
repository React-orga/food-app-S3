                ...
                <div className="relative mx-auto flex max-w-screen-lg flex-col py-4 sm:flex-row sm:items-center sm:justify-between">
                    <a className="flex items-center text-2xl font-black" href="/">
                        <span className="mr-2 text-3xl text-blue-600">
                            <img className="mx-auto block h-24 rounded-full sm:mx-0 sm:shrink-0"src="/ReactShop_logo.webp" alt="" />
                        </span>
                    ...
                    <nav aria-label="Header Navigation" className="peer-checked:block hidden pl-2 py-6 sm:block sm:py-0">
                        <ul className="flex flex-col gap-y-4 sm:flex-row sm:gap-x-8">
                            {links.map((link) => (
                                <li key={link.href}>
                                    <Link className="text-gray-600 hover:text-blue-600" to={link.href}>{link.label}</Link>
                                </li>
                            ))}
                        </ul>
                    </nav>
